...
export function isJsonrcp(body: any): body is JsonrpcRequestBody {
  if (typeof body.jsonrpc !== 'string') return false;
  if (typeof body.id !== 'string' && typeof body.id !== 'number') return false;
  if (typeof body.method !== 'string') return false;
  if (!Array.isArray(body.params)) return false;
  return true;
}

export function isJsonrcpArray(body: any): body is Array<JsonrpcRequestBody> {
  let check = true;
